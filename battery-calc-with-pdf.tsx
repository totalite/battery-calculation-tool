...
export default function BatteryCalculator() {
  ...
  const calculatePower = useMemo(() => {
    ...
    // At 100% LED: 380s, At 10% LED: 400s
    const measurementDuration = 380 + ((100 - ledPower) / 90) * 20;
...
Average Power Draw: ${calculatePower.avgPower.toFixed(3)} W
...
Idle Power (standby): ${IDLE_POWER} W
Measurement Duration: ${formatTime(calculatePower.measurementDuration)}
Idle Duration per Cycle: ${formatTime(calculatePower.idleTime)}

Power Contribution:
  • Active (measurement): ${activePercent}%
  • Idle (standby): ${idlePercent}%
...
• These calculations are based on controlled test data from Fugro's 
  Tech Center in Nootdorp (September 2025).
