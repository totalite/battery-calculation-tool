...
import { Battery, Zap, Clock, AlertCircle, Info, Download } from 'lucide-react';
...
export default function BatteryCalculator() {
  ...
  // Fixed values
  const integrationCycles = 3;
  const voltage = 12;
  
  // Power consumption constants (measured at 12V DC)
  const STANDBY_LTE_POWER = 0.066;  // Standby power with LTE active (W)
  const TILT_SENSOR_COLD = 0.071;   // Additional power for tilt sensor heating in cold conditions (W)
  
  // Updated constants based on actual measurements (Sept 2025, Fugro Tech Center)
  // Active measurement power: Base = (0.00589 × LED%) + 0.66961
  // Derived from: 100% LED = 1.259W, 10% LED = 0.729W
  const SLOPE_3_CYCLES = 0.00589;
  const INTERCEPT_3_CYCLES = 0.66961;
  ...
  const calculatePower = useMemo(() => {
    ...
    // Measurement duration is constant at 400s based on actual measurements
    const measurementDuration = 400;
    
    // Calculate time spent in standby mode between measurements
    const standbyTime = Math.max(0, measurementInterval - measurementDuration);
    
    // Calculate average power over the full measurement interval
    const activePowerContrib = (basePower * measurementDuration) / measurementInterval;
    const standbyPowerContrib = (STANDBY_LTE_POWER * standbyTime) / measurementInterval;
    const avgPower = activePowerContrib + standbyPowerContrib;
  ...
  }, [ledPower, measurementInterval, tempCondition, lteCondition]);
  ...
  const standbyPercent = ((calculatePower.standbyPowerContrib / calculatePower.avgPower) * 100).toFixed(1);
  ...
  const generatePDF = () => {
    ...
    const pdfContent = `
TotaLite Battery Capacity Report
Generated: ${date}
...
Measurement Settings:
  • LED Power: ${ledPower}%
  • Integration Cycles: ${integrationCycles} cycle${integrationCycles > 1 ? 's' : ''}
  • Measurement Interval: ${formatInterval(measurementInterval)}
  • Input Voltage: ${voltage}V DC

Deployment Conditions:
  • Duration: ${deploymentDays} days
  • Temperature: ${tempCondition === 'cold' ? 'Cold (<10°C)' : 'Normal (10-40°C)'}
  • LTE Coverage: ${lteCondition === 'good' ? 'Good' : 'Poor'}
...
Required Capacity: ${batteryCalc.requiredCapacityAh.toFixed(1)} Ah at ${voltage}V
Total Energy: ${batteryCalc.requiredCapacityWh.toFixed(0)} Wh (with 30% safety margin)

Recommended Battery: ${recommendedAh} Ah or higher
...
Peak Power (during measurement): ${calculatePower.basePower.toFixed(3)} W
Standby Power (LTE active): ${STANDBY_LTE_POWER} W
Measurement Duration: ${formatTime(calculatePower.measurementDuration)}
Standby Duration per Cycle: ${formatTime(calculatePower.standbyTime)}

Power Contribution:
  • Active (measurement): ${activePercent}%
  • Standby (LTE active): ${standbyPercent}%
...
Total Measurements: ${Math.round(batteryCalc.totalMeasurements).toLocaleString()}
Energy per Measurement: ${batteryCalc.energyPerMeasurement.toFixed(4)} Wh
Total Energy (no margin): ${batteryCalc.totalEnergyWh.toFixed(1)} Wh
Safety Margin Applied: 30%
...
Battery Selection:
Select a battery with capacity of at least ${batteryCalc.requiredCapacityAh.toFixed(1)} Ah 
at ${voltage}V (${batteryCalc.requiredCapacityWh.toFixed(0)} Wh total).

For optimal performance and longevity, we recommend using a battery 
rated for ${recommendedAh} Ah or higher.

Target Distance Guide: ${getDistanceGuide(ledPower)}
...
• These calculations are based on controlled test data from Fugro's 
  Tech Center in Nootdorp (September 2025). Power consumption formulas
  are calibrated from actual measurements: 100% LED = 1.259W, 10% LED = 0.729W.
  
• Standby power (0.066W) accounts for LTE connectivity between measurements.
  This ensures the equipment can transmit data while in standby mode.
  
• The 30% safety margin accounts for battery degradation over time 
  and operational reserves.
  
• Actual power consumption may vary based on specific site conditions,
  ambient temperature, and LTE signal quality.
  
• For mission-critical deployments, consider additional battery 
  capacity or backup power systems.
  
• Formula calibration date: November 2025 (verified against raw measurement data).

===============================================================================

Report generated by TotaLite Battery Calculator
© ${new Date().getFullYear()} TotaLite - Precision Surveying Systems
`;
    ...
    a.href = url;
    a.download = `TotaLite_Battery_Report_${date.replace(/\//g, '-')}.txt`;
        ...
        <div className="bg-blue-50 border-l-4 border-blue-600 p-4 mb-6 rounded">
          <div className="flex items-start gap-2">
            ...
            <p className="text-sm text-slate-700">
              ...
              environmental conditions, and deployment duration. Calculations account for both active measurement power 
              and standby power (with LTE active) between measurements. Includes 30% safety margin for battery degradation and reserves.
            </p>
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="bg-slate-50 rounded-lg p-6">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-slate-600">Standby power (LTE):</span>
                  <span className="font-medium">{STANDBY_LTE_POWER} W</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  <span className="text-slate-600">Standby duration:</span>
                  <span className="font-medium">{formatTime(calculatePower.standbyTime)}</span>
                </div>
                ...
                <div className="flex justify-between">
                  <span className="text-slate-600">Standby contribution:</span>
                  <span className="font-medium">{standbyPercent}%</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-50 rounded-lg p-6">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{Math.round(batteryCalc.totalMeasurements).toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
